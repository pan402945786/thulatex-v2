...
具体方法如算法\ref{algorithm:reverseReset}所示。
\begin{algorithm}
	...
	\begin{algorithmic}[1]
        ...
        \FOR {$i \to totalLayers$}
            % \STATE $accRetain,accForget \gets reverseResetInnerCycle(totalLayers, i, C_{forget}, $\\$D_{retain\_test}, D_{forget\_test}, modelNormal, modelInit)$ (算法\ref{algorithm:reverseResetInnerCycle})
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
        ...
        \FOR {$epoch \to EPOCH$}
            ...
            \ENDFOR
            \IF {$(loss\_sum \div dataLength < T_{threshold}) and (T\_converge == null)$}
